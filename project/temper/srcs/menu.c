  ...
  if (selected)
  {
    ...
    if (highlight_current)
    {
      print_string((char *)selected_char, make_color16(0x1F, 0x3F, 0x1F),
                   make_color16(0x0, 0x30, 0x0), draw_offset + (input->segment_position * 8), y_offset, wkRESOLUTION_WIDTH);
    }
  }
  else
  {
                ...
                menu_option_struct *menu_option, u32 selected)
{
  ...
  draw_input_str(menu_state, input, menu_state->current_menu->column_start + (display_input_offset * 8), selected, 1);
}
    ...
    if (separator_str[0] && (i != multi_input_int->num_input_ints - 1))
    {
      if (selected)
      {
        print_string(separator_str, make_color16(0x1F, 0x3F, 0x1F),
                     make_color16(0x0, 0x0, 0x17), menu_state->current_menu->column_start + (display_input_offset * 8), menu_option->line_number * 8,
                     wkRESOLUTION_WIDTH);
      }
      else
      {
        print_string_bg(separator_str, make_color16(0x1F, 0x3F, 0x1F),
                        menu_state->current_bg, menu_state->current_menu->column_start + (display_input_offset * 8), menu_option->line_number * 8,
                        wkRESOLUTION_WIDTH);
      }
                           ...
                           menu_option_struct *menu_option, u32 focus_type)
{
  if (focus_type == FOCUS_TYPE_EXIT)
  {
    ...
    for (i = 0; i < multi_input_int->num_input_ints; i++)
    {
      ...
      ((menu_option_struct *)current_input_int)->focus_function(menu_state, (menu_option_struct *)current_input_int, focus_type);
...
menu_option_select_struct *create_select(menu_option_select_struct
                                             *select,
                                         char *name, u32 line_number, select_function_type select_function)
{
...
menu_option_input_int_struct *create_input_int(menu_option_input_int_struct
                                                   *input_int,
                                               char *name, u32 line_number, s32 *value, u32 num_segments,
...
menu_struct *create_menu(u32 num_options, menu_struct *parent_menu,
                         draw_menu_function_type draw_function, focus_menu_function_type focus_function)
{
