                      ...
                      u32 screen_limit)
{
  ...
  while (current_char)
  {
    ...
    else
    {
      ...
      {
        goto drawend; // returnで抜けようとしていたのでgoto で終了処理に飛ばすことにした
        //return;
      }
...
{
  print_string_ext(str, fg_color, bg_color, x, y, NULL, 0, screen_limit);
}

void print_string_bg(const char *str, u32 fg_color, u16 *bg_ptr,
                     u32 x, u32 y, u32 screen_limit)
{
  print_string_ext(str, fg_color, 0x10000, x, y, bg_ptr, 0, screen_limit);
}
...
{
  buffer_quarter_intensity(_dest);
}

void copy_screen_half_intensity(u16 *_dest)
{
  void *ptr = get_screen_ptr();
  u32 pitch = get_screen_pitch();
  buffer_half_intensity(_dest, ptr, pitch);
}
